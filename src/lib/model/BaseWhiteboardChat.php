...
abstract class BaseWhiteboardChat extends BaseObject  implements Persistent {
  ...
  const PEER = 'WhiteboardChatPeer';
	...
	public function setEndedAt($v)
	{
		...
		if ( $this->ended_at !== null || $dt !== null ) {
			...
			$currNorm = ($this->ended_at !== null && $tmpDt = new DateTime($this->ended_at)) ? $tmpDt->format('Y-m-d\\TH:i:sO') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d\\TH:i:sO') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->ended_at = ($dt ? $dt->format('Y-m-d\\TH:i:sO') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->started_at = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->ended_at = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->is_public = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			...
			return $startcol + 5; // 5 = WhiteboardChatPeer::NUM_COLUMNS - WhiteboardChatPeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BaseWhiteboardChat:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseWhiteboardChat:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseWhiteboardChat:save:pre') as $callable)
    {
    ...
    if ($this->isNew() && !$this->isColumnModified(WhiteboardChatPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseWhiteboardChat:save:post') as $callable)
    {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getStartedAt(),
			$keys[2] => $this->getEndedAt(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setStartedAt($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setEndedAt($arr[$keys[2]]);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseWhiteboardChat:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseWhiteboardChat::%s', $method));
    }

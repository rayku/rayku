...
/**
 * Base class that represents a row from the 'size' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseSize extends BaseObject  implements Persistent {
  ...
  const PEER = 'SizePeer';
	...
	/**
	 * Initializes internal state of BaseSize object.
	 * @see        applyDefaults()
	 */
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = SizePeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SizePeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = SizePeer::NUM_COLUMNS - SizePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Size object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = SizePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			$con->commit();
			SizePeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += SizePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = SizePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = SizePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = SizePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SizePeer::ID)) $criteria->add(SizePeer::ID, $this->id);
		if ($this->isColumnModified(SizePeer::NAME)) $criteria->add(SizePeer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(SizePeer::ID, $this->id);
	 ...
	 * Otherwise if this Size has previously been saved, it will retrieve
	 * related Items from storage. If this Size is new, it will return
		...
		if ($this->collItems === null) {
			if ($this->isNew()) {
			   $this->collItems = array();
				...
				$criteria->add(ItemPeer::SIZE_ID, $this->id);
	...
	public function getItemsJoinItemType($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collItems === null) {
			if ($this->isNew()) {
				...
				$this->collItems = ItemPeer::doSelectJoinItemType($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last

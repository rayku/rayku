...
/**
 * Base class that represents a row from the 'assignment' table.
 *
 * 
 *
  ...
  const PEER = 'AssignmentPeer';
	...
	public function setClassroomId($v)
	{
		...
		if ($this->classroom_id !== $v) {
			$this->classroom_id = $v;
			$this->modifiedColumns[] = AssignmentPeer::CLASSROOM_ID;
		}
	...
	public function setTitle($v)
	{
		...
		if ($this->title !== $v) {
			$this->title = $v;
			$this->modifiedColumns[] = AssignmentPeer::TITLE;
		}
	...
	public function setFormat($v)
	{
		...
		if ($this->format !== $v) {
			$this->format = $v;
			$this->modifiedColumns[] = AssignmentPeer::FORMAT;
		}
	...
	public function setAttachments($v)
	{
		...
		if ($this->attachments !== $v) {
			$this->attachments = $v;
			$this->modifiedColumns[] = AssignmentPeer::ATTACHMENTS;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = AssignmentPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = AssignmentPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function setDueDate($v)
	{
		...
		if ( $this->due_date !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = AssignmentPeer::DUE_DATE;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->classroom_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->title = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->description = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->format = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->attachments = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			...
			return $startcol + 9; // 9 = AssignmentPeer::NUM_COLUMNS - AssignmentPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			AssignmentPeer::doDelete($this, $con);
	...
	{
    if ($this->isNew() && !$this->isColumnModified(AssignmentPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
    }

    if ($this->isModified() && !$this->isColumnModified(AssignmentPeer::UPDATED_AT))
    {
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = AssignmentPeer::doInsert($this, $con);
					...
					$affectedRows += AssignmentPeer::doUpdate($this, $con);
				}
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getClassroomId(),
			$keys[2] => $this->getTitle(),
			$keys[3] => $this->getDescription(),
			$keys[4] => $this->getFormat(),
			$keys[5] => $this->getAttachments(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setClassroomId($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setTitle($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setDescription($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setFormat($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setAttachments($arr[$keys[5]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AssignmentPeer::CLASSROOM_ID)) $criteria->add(AssignmentPeer::CLASSROOM_ID, $this->classroom_id);
		if ($this->isColumnModified(AssignmentPeer::TITLE)) $criteria->add(AssignmentPeer::TITLE, $this->title);
		if ($this->isColumnModified(AssignmentPeer::DESCRIPTION)) $criteria->add(AssignmentPeer::DESCRIPTION, $this->description);
		if ($this->isColumnModified(AssignmentPeer::FORMAT)) $criteria->add(AssignmentPeer::FORMAT, $this->format);
		if ($this->isColumnModified(AssignmentPeer::ATTACHMENTS)) $criteria->add(AssignmentPeer::ATTACHMENTS, $this->attachments);
		if ($this->isColumnModified(AssignmentPeer::CREATED_AT)) $criteria->add(AssignmentPeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(AssignmentPeer::UPDATED_AT)) $criteria->add(AssignmentPeer::UPDATED_AT, $this->updated_at);
		if ($this->isColumnModified(AssignmentPeer::DUE_DATE)) $criteria->add(AssignmentPeer::DUE_DATE, $this->due_date);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setAttachments($this->attachments);
	...
	public function getClassroom(PropelPDO $con = null)
	{
		if ($this->aClassroom === null && ($this->classroom_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aClassroom->addAssignments($this);
			 */
		}
	 ...
	 * Otherwise if this Assignment has previously been saved, it will retrieve
	 * related Submissions from storage. If this Assignment is new, it will return
		...
		if ($this->collSubmissions === null) {
			if ($this->isNew()) {
			   $this->collSubmissions = array();
				...
				$criteria->add(SubmissionPeer::ASSIGNMENT_ID, $this->id);
	 ...
	 */
	public function getSubmissionsJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(AssignmentPeer::DATABASE_NAME);
		}
		...
		if ($this->collSubmissions === null) {
			if ($this->isNew()) {
				...
				$this->collSubmissions = SubmissionPeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last

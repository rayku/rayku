...
abstract class BaseAlbum extends BaseObject  implements Persistent {
  ...
  const PEER = 'AlbumPeer';
	...
	public function setOwnerId($v)
	{
		...
		if ($this->owner_id !== $v) {
			$this->owner_id = $v;
			$this->modifiedColumns[] = AlbumPeer::OWNER_ID;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = AlbumPeer::CREATED_AT;
			}
	...
	{
    if ($this->isNew() && !$this->isColumnModified(AlbumPeer::CREATED_AT))
    {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AlbumPeer::OWNER_ID)) $criteria->add(AlbumPeer::OWNER_ID, $this->owner_id);
		if ($this->isColumnModified(AlbumPeer::CREATED_AT)) $criteria->add(AlbumPeer::CREATED_AT, $this->created_at);
	 ...
	 * related Pictures from storage. If this Album is new, it will return
		...
		if ($this->collPictures === null) {
			if ($this->isNew()) {
			   $this->collPictures = array();
				...
				$criteria->add(PicturePeer::ALBUM_ID, $this->id);
	 ...
	 */
	public function getPicturesJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(AlbumPeer::DATABASE_NAME);
		}
		...
		if ($this->collPictures === null) {
			if ($this->isNew()) {
				...
				$this->collPictures = PicturePeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last

...
$query = mysql_query("select * from user where username='".$name[3]."' ", $connection, $connection) or die(mysql_error());
...
$this->lastSessions = WhiteboardChatPeer::doSelect($cLastSessions);
    ...
    public function executeDirect()
    {
        ...
        mysql_query("INSERT INTO `user_expert` (`user_id`, `checked_id`, `category_id`, `question`, `exe_order`, `time`, `status`) VALUES ('".$userId."', '".$_GET['id']."', '5', 'To be discussed', 1, '".$time."', 1) ", $connection) or die(mysql_error());
    ...
    public function executeList()
    {
        ...
        if(!empty($_POST['hidden'])) {
            ...
            for($i=0; $i < $count; $i++) {
                ...
                mysql_query("INSERT INTO `user_expert` (`user_id`, `checked_id`, `category_id`, `question`, `exe_order`, `time`, status, close) VALUES ('".$userId."', '".$_POST['checkbox'][$i]."', '".$_SESSION['subject']."', '".$_SESSION['question']."','".(++$j)."', '".$time."', 1, ".$close.") ", $connection) or die(mysql_error());

...
foreach ( $categories as $category) {
    if (in_array($category->getId(), $limitSubject)) {
?>
                <option value="<?=$category->getId();?>" <?php if ($cat == $category->getId()) { ?> selected="selected" <?php } ?> >
                <?=$category->getName();?>
                </option>
<?php
    }
...
function setStatus(value)
{
    ...
    document.cookie = "onoff"+ "=" +value;
        ...
        <select name="school" onchange="return setSchool(this.value)" style="background:none;padding:4px;height:auto;border:1px solid #CCC">
          <option value="">---- SELECT ----</option>
          <option value="utoronto" <?php if ($_COOKIE["school"] == "utoronto") { ?> selected="selected" <?php } ?> >University of Toronto</option>
          <option value="ubc" <?php if ($_COOKIE["school"] == "ubc") { ?> selected="selected" <?php } ?> >University of British Columbia</option>
        </select>

...
?>
  ...
  <img src="/images/expert_saved.png" alt="Rayku Staff" title="Rayku Staff" style="border:none;margin:5px 3px 0 0;" />
  <?php else: ?>
  <?php if($row['score'] >= 1000 && $row['score'] < 2000) { ?>
  <div style="width:125px;" align="center">
  <img src="/images/beside1.gif" alt="Certified Tutor" title="Certified Tutor" /><br />
...
$ij = 1; $curr_user_rank = '';
    ...
    <?php    $query_usr = mysql_query("select * from user where id=".$logedUserId." ") or die(mysql_error());
...
if(!empty($_logedUserId)): 
	...
	if($sf_user->getRaykuUser()->getType()==5 || $sf_user->getRaykuUser()->getType()==4):
         ...
         $query = mysql_query("select * from thread where id = ".$thread->getId()."") or die(mysql_error());
...
if(!empty($_logedUserId)): 
		 ...
		 $query_fm = mysql_query("select * from expert_subscribers where expert_id=".$logedUserId." and user_id=".$_logedUserId) or die(mysql_error());
...
?>
      ...
      <?php	$con = mysql_connect("localhost", "rayku_db", "db_*$%$%");
			...
			id = ".$thread->getId()." and reported=1") or die(mysql_error());

...
$query = mysql_query("select * from user_score where user_id =".$this->post->getPosterId()) or die(mysql_error());
$row = mysql_fetch_assoc($query);

$newScore = $row['score'] + 25 ;

mysql_query("update user_score set score = ".$newScore." where user_id =".$this->post->getPosterId()) or die(mysql_error());
	...
	public function executeThread()
	{
		...
		if($_GET['follow']<>'' and $_GET['expert_id']<>'')
		{
					...
					$query = mysql_query("select * from expert_subscribers where
					 expert_id = ".$expertid." and user_id =".$userid) or die(mysql_error());
					
					if(mysql_num_rows($query) == 0) {
					
					mysql_query("insert into expert_subscribers(expert_id, user_id)
					 values('".$expertid."', '".$userid."')") or die(mysql_error());

					$queryScore = mysql_query("select * from user_score where user_id =".$expertid) or die(mysql_error());
					$rowScore = mysql_fetch_assoc($queryScore);
					$newScore = '';

					$newScore = $rowScore['score'] + 10 ;

					mysql_query("update user_score set score = ".$newScore." where user_id =".$expertid) or die(mysql_error());
			...
			$curr_date=date("Y-m-d H:i:s");
				if($_GET['thread_id']<>'')
				{
					...
					$query = mysql_query("select * from thread where
					 id = ".$thread_id." and  reported=1") or die(mysql_error());
					
					if(mysql_num_rows($query) == 0) {
					
					mysql_query("update thread set reported=1,reported_date='".$curr_date."' where id=".$thread_id) or die(mysql_error());
				...
				if($_GET['post_id']<>'')
				{
					...
					$query = mysql_query("select * from post where
					 id = ".$post_id." and  reported=1") or die(mysql_error());
					
					if(mysql_num_rows($query) == 0) {
					
					mysql_query("update post set reported=1 ,reported_date='".$curr_date."' where id=".$post_id) or die(mysql_error());
	...
	*/
	...
	//	}
		...
		if(!is_numeric($this->getRequestParameter('thread_id')))
		{
			...
			{
				if($tag == 'I') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'a') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'about') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'an') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'are') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS; }
				if($tag == 'as') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'at') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'be') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS; }
				if($tag == 'by') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'com') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'de') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'en') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'for') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'from') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'how') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS; }
				if($tag == 'in') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'is') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'it') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'la') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'of') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'on') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS; }
				if($tag == 'or') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS; }
				if($tag == 'that') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'the') { $this->msg = $tag.' is a stop word, please add the proper tag';return sfView::SUCCESS; }
				if($tag == 'this') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'to') { $this->msg = $tag.' is a stop word, please add the proper tag';return sfView::SUCCESS; }
				if($tag == 'was') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'what') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'when') { $this->msg = $tag.' is a stop word, please add the proper tag';return sfView::SUCCESS; }
				if($tag == 'where') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'who') { $this->msg = $tag.' is a stop word, please add the proper tag';return sfView::SUCCESS; }
				if($tag == 'will') { $this->msg = $tag.' is a stop word, please add the proper tag';return sfView::SUCCESS; }
				if($tag == 'with') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'und') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}
				if($tag == 'the') { $this->msg = $tag.' is a stop word, please add the proper tag';return sfView::SUCCESS; }
				if($tag == 'www') { $this->msg = $tag.' is a stop word, please add the proper tag'; return sfView::SUCCESS;}				
			}
							
			//Make the thread or display an error if there was an issue
      $thread = $user->makeNewThread(
              $this->getRequestParameter('forum_id'),
              $this->getRequestParameter('thread_title'),
              $this->getRequestParameter('post_body'),
              $this->getRequestParameter('category_id'),
              $this->getRequestParameter('notify_email'),
              $this->getRequestParameter('notify_pm'),
              $this->getRequestParameter('tags'),
			  $this->getRequestParameter('school_grade')
              );
			if(!$thread)
			{
			...
			
			{
				...
				if($theard_title_len>=10 and $theard_body_len>=10)
				{
					...
					foreach($arrayoftags as $tag) 
					{
					  ...
					  $this->getRequestParameter('tags'),
					  $this->getRequestParameter('school_grade'),
					  $this->getRequestParameter('stickie')
					  ); 
			...
			if($this->getRequestParameter('stickie') == 1) 
			{
				...
				$stickie = 1;
					...
					///////////////////updating the ip of the user
						...
						mysql_query("update thread set user_ip='".$_SERVER['REMOTE_ADDR']."', stickie = ".$stickie." where id=".$latest[0]->getThreadId()."");
...
$_quick_reply .= "</div>";
				...
				$user->makeNewPost($this->getRequestParameter('thread_id'), $_quick_reply); 
					...
					///////////////////updating the ip of the user
						...
						$post_id=mysql_fetch_row(mysql_query("SELECT max(id) from post limit 0,1"));
						
						
						mysql_query("update post set 	user_ip='".$_SERVER['REMOTE_ADDR']."' where id=".$post_id[0]."");

...
$count = count($_POST['course_subject']);
	...
	for($i=0; $i < $count; $i++)
	{
		...
		if(!empty($_POST['course_subject'][$i]))
			...
			mysql_query("insert into user_course(user_id,course_subject,course_name,course_year,course_performance) values('".$user->getId()."','".$_POST['course_subject'][$i]."','".$_POST['course_name'][$i]."','".$_POST['course_year']."','".$_POST['grade'][$i]."')") or die(mysql_error());

		$query = mysql_query("select * from expert_category where user_id=".$user->getId()) or die(mysql_error());
					...
					if(!in_array($_POST['course_subject'][$i], $expertCategory)) {
						...
						mysql_query("insert into expert_category(user_id,category_id) values('".$user->getId()."','".$_POST['course_subject'][$i]."')") or die(mysql_error());

...
$_SESSION[$this->user->getUsername()] = time();
		...
		if(!empty($userId)) :
			...
			mysql_query("delete from popup_close where user_id=".$userId) or die(mysql_error());
			mysql_query("delete from sendmessage where asker_id =".$userId) or die(mysql_error());
			mysql_query("delete from user_expert where checked_id=".$userId) or die(mysql_error());
		endif;

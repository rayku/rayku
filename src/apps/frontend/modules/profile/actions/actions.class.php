...
//============================================================Modified By DAC021===============================================================================//
  ...
  if(!empty($_POST['course_new_sub']))
	...
	for($i=0; $i < $count; $i++)
	{
		...
		mysql_query("update user_course set course_name = '".$_POST['course_name'][$i]."',course_year = '".$_POST['course_year']."',course_performance = '".$_POST['grade'][$i]."' where user_id=".$logedUserId." and course_subject = '".$_POST['course_new_sub'][$i]."' and id=".$_POST['courseid'][$i]) or die(mysql_error());
  ...
  if(!empty($_POST['course_new_subject']))
     {
			...
			mysql_query("insert into user_course(user_id,course_subject,course_name,course_year,course_performance) values(".$logedUserId.",'".$_POST['course_new_subject']."','".$_POST['course_new_name']."','".$_POST['course_year']."','".$_POST['new_grade']."')") or die(mysql_error());

					$queryCategory = mysql_query("select * from expert_category where user_id=".$logedUserId) or die(mysql_error());
					...
					if(!in_array($_POST['course_new_subject'], $expertNewCategory)) {
						...
						mysql_query("insert into expert_category(user_id,category_id) values(".$logedUserId.",".$_POST['course_new_subject'].")") or die(mysql_error());
  ...
  if(!empty($_POST['subject']))
     {
		...
		for($j=0; $j < $countNew; $j++)
		{
			  if(!empty($newSubject[$j]))
			     {
					mysql_query("insert into user_course(user_id,course_subject,course_name,course_year,course_performance) values(".$logedUserId.",'".$newSubject[$j]."','".$newName[$j]."','".$_POST['course_year']."','".$newGrade[$j]."')") or die(mysql_error());

					$query = mysql_query("select * from expert_category where user_id=".$logedUserId) or die(mysql_error());
	...
	public function executeRowdelete()
		{
			...
			$queryNew = mysql_query("select * from user_course where id=".$newId[4]) or die(mysql_error());
			$rowNew = mysql_fetch_assoc($queryNew);

			$queryDelete = mysql_query("select * from user_course where course_subject=".$rowNew['course_subject']." and user_id=".$logedUserId) or die(mysql_error());
	...
	public function executeDelete()
		{
			...
			// For delete Shout
			$querynew = mysql_query("delete from shout where id=".$newCommentId[4]." ") or die(mysql_error());

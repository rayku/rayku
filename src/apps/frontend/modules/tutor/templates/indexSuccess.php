...
<?php
    ...
    <div id="facts">
<?php	
	$_query_scrore = mysql_query("select score from user_score where user_id=".$expert->getId()." ", $connection) or die(mysql_error());
	$rating_count = 0;
	$avg_rating = 0;
	$rating_score = mysql_fetch_row($_query_scrore);
	if($expert->getType() == 5 && $expert->isTutorStatusEnabled()) {
		?>
...
<?php 
	if(!empty($currentUser)) {
		...
		if($_currentUserType == 5) {
			?>
<?php
 $expertscor_qry = mysql_query("select score from user_score where user_id =".$expert->getId()." ", $connection) or die("error1");$expertscore_result = mysql_fetch_array($expertscor_qry); ?>
      <div class="expert-score">*Expert Score: <?=$expertscore_result['score']?></div>
      <?php  } ?>
...
<?php
	...
	if(count($lastSessions)>0) {
		$count_session=count($lastSessions)>3?3:
		count($lastSessions);
...
<?php
 if($expert->isTutorStatusEnabled()) { ?>
        <!--thumbnail wrap ends-->
    <div id="ratings">
      <h4 style="margin-bottom:20px; font-weight:bold; color: #575757;">Latest  Ratings by Students</h4>
<?php
	$rating_count = 0;
...
<?php	
	$_query = mysql_query("select * from whiteboard_chat where expert_id =".$expert->getId()." and rating !='' ", $connection) or die(mysql_error());

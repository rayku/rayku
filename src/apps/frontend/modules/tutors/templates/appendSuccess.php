...
<div id="resultpage">
	...
	<div class="cn-result" id="<?php echo 'first'.$xy; ?>">
		<div id="<?php echo $xy.'.1'; ?>" class="cn-column-one"
			onclick="rowCheck(this.id)"
			style="padding-right: 15px; width: 500px;">
			<p id="<?php echo $xy.'.2'; ?>" class="cn-title"
				onclick="rowCheck(this.id)">
			
			
			<div id="<?php echo $xy.'.3'; ?>" class="cn-user-info"
				onclick="rowCheck(this.id)"
				style="float: right; width: 150px; line-height: 14px" align="right">
				<strong style="color: #069"></strong>
				<?php if ($onlinecheck == "online") { ?>
				<?php if ($experts->getType() == 5) { ?>
				<img
					src="<?php echo sfConfig::get('app_rayku_url') ?>/images/expert_saved.png"
					alt="Rayku Staff" />
				<?php } ?>
				<a href="/tutor/<?php echo $experts->getUsername()?>"
					target="_blank" style="color: #8FAFC8"><?php echo $experts->getName()?>
					<span class="onlinenow">(online)</span> </a>
				<?php } else { ?>
				<?php if ($experts->getType() == 5) { ?>
				<img
					src="<?php echo sfConfig::get('app_rayku_url') ?>/images/expert_saved.png"
				...
				<a href="/tutor/<?php echo $experts->getUsername()?>"
					target="_blank" style="color: #8FAFC8"><?php echo $experts->getName()?>
					<span class="offlinenow">(offline)</span> </a>
				<?php } ?>
			</div>
			<?php if (($experts->getType() == 5)) : ?>
			<div
				style="float: left; height: 50px; line-height: 20px; width: 50px; border-right: 1px solid #CFD0D2;"
				align="center">
				<strong>#<?php echo $curr_user_rank; ?>
				</strong>
			</div>
			...
			<div
				...
				align="center">
				#
				<?php echo $curr_user_rank; ?>
			</div>
			<?php endif; ?>
			<div style="padding-left: 60px;">
				<u><a href="javascript:void(0);" title="Click to Select"><?php echo $allsub;?>
				</a> </u>
			</div>
			</p>
		</div>
...
?>
				<a href="/message/compose/<?php echo $experts->getUsername(); ?>"><img
					alt="in session"
					src="<?php echo image_path('em-busy.jpg', false); ?>"> </a>
				<?php
            } else {
...
?>
				<input type="checkbox" <?php if ($is_toronto_tutor){ ?>
					onchange="toronto_tutor=this.checked;console.log(toronto_tutor);"
					<?php } ?> name="checkbox[]" id="checkbox_<?php echo $xy?>"
					value="<?php echo $newOne['userid']; ?>"
					onclick="setvalue(this.id)"
					style="background-color: #DEF3FE; border: 1px solid red;"
					<?php echo (@$cookiy==$newOne['userid'])?"checked='checked'":""; ?> />
...
?>
				<a href="/message/compose/<?php echo $experts->getUsername(); ?>"><img
					height="18" width="59" alt=""
					src="<?php echo image_path('em-email.jpg', false); ?>"> </a>
				<?php } ?>
...
$_v++;
	...
	<p class="cn-pricepermin" align="center" style="margin-top: 10px">No
		online tutors found for this category with the criteria of School
		level....</p>
	<?php } else {?>
	<p class="cn-pricepermin" align="center" style="margin-top: 10px">No
		online tutors found for this category.... ..</p>
	<?php }
    }  else if ($_COOKIE["onoff"] == 2) {

        if (!empty($_COOKIE["school"])) { ?>
	<p class="cn-pricepermin" align="center" style="margin-top: 10px">No
		offline tutors found for this category with the criteria of School
		level....</p>
	<?php } else { ?>
	<p class="cn-pricepermin" align="center" style="margin-top: 10px">No
		offline tutors found for this category....</p>
	<?php }

    } else {
        if (!empty($_COOKIE["school"])) { ?>
	<p class="cn-pricepermin" align="center" style="margin-top: 10px">No
		tutors found for this category with the criteria of School Level....</p>
	<?php }   else {  ?>
	<p class="cn-pricepermin" align="center" style="margin-top: 10px">No
		offline tutors found for this category....</p>
	<?php } ?>
	...
	<p class="cn-pricepermin" align="center" style="margin-top: 10px">No
		tutors found</p>
	<p class="cn-pricepermin" align="center"
		style="margin-top: 10px; color: #C30">Please select a category from
		the category list</p>
	<div class="clear-both"></div>
	<?php } ?>
</div>
...
?>
<div style="width: 100%; font-size: 20px; line-height: 35px;"
	align="right">
	<div id="bottomMoreButton">
		<img src="<?php echo image_path('ajax-loader.gif', false); ?>"
			style="display: none" class="spinner" /> <a
			id="more_<?php echo @$next_records?>" class="more_records" name="2"
			href="javascript: void(0)">show more listings</a>
	</div>
</div>
<?php 	}
        }

...
<?php include_partial( 'demo_list', array( 'expert_cats' => $expert_cats, 'cat' => $cat, 'tutorsCount' => $tutorsCount, 'userId' => $userId, 'newOnlineUser' => $newOnlineUser, 'newOfflineUser' => $newOfflineUser, '_checkOnlineUsers' => $_checkOnlineUsers, 'course_id' => $course_id, 'rankCheckUsers' => $rankCheckUsers) ); ?>
</div>
<br/>
	 ...
	 rayku_tutor_pag('a.more_records').livequery("click", function(e){
		  ...
		  rayku_tutor_pag('#bottomMoreButton .spinner').show();
			...
			rayku_tutor_pag.post(file+"?show_more_post="+next+'&x='+keepID+'&p='+posted_on, {
				...
				rayku_tutor_pag('.cn-content').html(rayku_tutor_pag(response));
			});
		});	
</script>

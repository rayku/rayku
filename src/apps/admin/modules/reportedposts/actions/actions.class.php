...
class reportedpostsActions extends sfActions
{
  public function executeIndex()
  {
	...
	$con = mysql_connect("localhost", "rayku_db", "db_*$%$%");
		...
		// echo "update thread set banned=1  where id=".$_GET['ban_id'];
		 
		 if($_GET['del_id']<>'')
		 {
			 
			 $_query = mysql_query("delete from thread where id=".$_REQUEST['del_id']) ;
		 }
		  if($_GET['un_report_id']<>'')
		 {
			 $_query = mysql_query("update thread set reported=0  where id=".$_REQUEST['un_report_id']) ;
		 }
...
$_query = mysql_query("select * from thread  where reported=1 order by  reported_date desc limit $start,10");
...
$i = 0;
	...
	$this->repthreads = $repthreads;
		 ...
		 if($_GET['del_post_id']<>'')
		 {
			 ...
			 $_query = mysql_query("delete from post where id=".$_REQUEST['del_post_id']) ;
		 }
		  if($_GET['un_report_post_id']<>'')
		 {
			 $_query = mysql_query("update post set reported=0  where id=".$_REQUEST['un_report_post_id']) ;
		 }
...
$_query_post = mysql_query("select * from post  where reported=1 order by  reported_date desc limit $start,10") ;

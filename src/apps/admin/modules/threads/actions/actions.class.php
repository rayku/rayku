	...
	$con = mysql_connect("localhost", "rayku_db", "db_*$%$%");
		...
		// echo "update thread set banned=1  where id=".$_GET['ban_id'];
		 ...
		 if($_GET['ban_id']<>'')
		 {
			 ...
			 $_query = mysql_query("update thread set banned=1  where id=".$_REQUEST['ban_id']) or die(mysql_error());
		 }
		  if($_GET['unban_id']<>'')
		 {
			 $_query = mysql_query("update thread set banned=0  where id=".$_REQUEST['unban_id']) or die(mysql_error());
		 }
...
$_query = mysql_query("select * from thread  order by  created_at desc limit $start,10") or die(mysql_error());
